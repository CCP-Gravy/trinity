...
bool Tr2JpgHandler::ReadHeader()
{
	CCP_STATS_ZONE( __FUNCTION__ );

	m_compressedData.resize( m_source->GetSize() );
	if( m_compressedData.empty() )
	{
		return false;
	}

	m_source->Read( &m_compressedData[0], m_compressedData.size() ); 
...
bool Tr2JpgHandler::IsSupported()
{
	return true;
}
...
unsigned Tr2JpgHandler::GetOffset( unsigned mipLevel, unsigned face ) const
{
	return 0;
}
...
bool Tr2JpgHandler::Save( const Tr2HostBitmap& image, IBlueStream* output )
{	
	CCP_ASSERT( output );
	...
	else
	{
		...
		for( unsigned j = 0; j != image.GetHeight(); ++j )
		{
			const unsigned char* src = (const unsigned char*)image.GetRawData( 0, j );
			...
			if( is1010102 )
			{
				for( unsigned i = 0; i != image.GetWidth(); ++i, dst += 3, src += 4 )
				{
					...
					dst[bSwizzle] = ( ( rgb10 >> 20 ) & 1023 ) >> 2;
					dst[1] = ( ( rgb10 >> 10 ) & 1023 ) >> 2;
					dst[rSwizzle] = ( ( rgb10 >>  0 ) & 1023 ) >> 2;
				}
			}
			else
			{
