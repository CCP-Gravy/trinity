...
// --------------------------------------------------------------------------------------
bool Tr2BmpHandler::ReadHeader()
{
	m_fileSize = m_source->GetSize();

	if( m_source->Read( &m_bmpHeader, sizeof( BmpHeader ) ) != sizeof( BmpHeader ) )
	{
		return false;
	}
	...
	if( m_source->Read( &m_dibHeader, sizeof( MinDibHeader ) ) != sizeof( MinDibHeader ) )
	{
		return false;
	}
	...
	m_source->Seek( m_bmpHeader.offBits - sizeof( DibHeader ) - sizeof( BmpHeader ), BS_CURRENT );
...
//   true Always
// --------------------------------------------------------------------------------------
bool Tr2BmpHandler::IsSupported()
{
	return true;
}
...
// --------------------------------------------------------------------------------------
unsigned Tr2BmpHandler::GetOffset( unsigned mipLevel, unsigned face ) const
{
	unsigned offset = 0;
...
//   false Otherwise
// --------------------------------------------------------------------------------------
bool Tr2BmpHandler::Save( const Tr2HostBitmap& image, IBlueStream* output )
{
	if( !image.IsValid() )
	{
	...
	if( dibHeader.bitCount == 32 )
	{
		for( unsigned j = 0; j < image.GetHeight(); ++j )
		{
			...
			{
				unsigned zero = 0;
				if( output->Write( &zero, padding ) != padding )
				{
	...
	else
	{
		...
		for( unsigned j = 0; j < image.GetHeight(); ++j )
		{
			...
			if( output->Write( destRow, destStride ) != destStride )
			{
